...
impl FromStr for CubeColorCount {
    ...
    fn from_str(input: &str) -> Result<Self, Self::Err> {
        ...
        match (_text.as_str(), _num) {
            ("red", Ok(_num)) => Ok(CubeColorCount { color: CubeColor::Red, count: _num }),
            ("green", Ok(_num)) => Ok(CubeColorCount { color: CubeColor::Green, count: _num }),
            ("blue", Ok(_num)) => Ok(CubeColorCount { color: CubeColor::Blue, count: _num }),
...
fn get_game_sets(game_set_str: &str) -> u32 {
    let cube_color_counts: Vec<_> = game_set_str
        .split(|c| c == ',' || c == ';')
        .map(CubeColorCount::from_str).collect();
